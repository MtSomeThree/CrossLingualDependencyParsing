...
from neuronlp2.models import Constraint
...
def main():
    ...
    args_parser.add_argument('--decode', choices=['mst', 'greedy', 'ilp'], default='mst', help='decoding algorithm')
    ...
    args_parser.add_argument('--train_constraints', type=str, choices=['binary', 'Lagrange', 'lagrange'], default='binary')
    args_parser.add_argument('--tolerance', type=float, default=0.01)
    #
    ...
    constraints_method = args.train_constraints
    ...
    VERB_idx = pos_alphabet.get_index('VERB')
    AUX_idx = pos_alphabet.get_index('AUX')
    NOUN_idx = pos_alphabet.get_index('NOUN')
    ADP_idx = pos_alphabet.get_index('ADP')
    ADJ_idx = pos_alphabet.get_index('ADJ')
    DET_idx = pos_alphabet.get_index('DET')
    PRON_idx = pos_alphabet.get_index('PRON')
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        '''
        ################################################################################################
        #if epoch == args.check_dev + 1:
	#    exit()
        
        if epoch % args.check_dev != 0:
            continue
        if epoch == args.check_dev:
            print ('Load model from %s'%(model_name))
        ...
        constraints = None
        #constraints.append(Constraint(VERB_idx, PRON_idx, 0.305))
        #constraints.append(Constraint(ADJ_idx, NOUN_idx, 0.337))
        #constraints.append(Constraint(VERB_idx, NOUN_idx, 0.3366))
        #constraints.append(Constraint(NOUN_idx, ADJ_idx, 0.15))
        #constraints.append(Constraint(NOUN_idx, DET_idx, 0.0))
        #constraints.append(Constraint(NOUN_idx, VERB_idx, 0.4134))
        #constraints.append(Constraint(NOUN_idx, NOUN_idx, 0.186))
        #constraints.append(Constraint(ADJ_idx, ADJ_idx, 0.269))
        ...
        train_constraints(arc_list, type_list, length_list, pos_list, constraints, tolerance)
        
        for constraint in constraints:
            network.train_constraint(arc_list, type_list, length_list, pos_list, constraint)
        '''
        for batch in conllx_data.iterate_batch_variable(data_dev, batch_size):
            word, char, pos, heads, types, masks, lengths = batch
            heads_pred, types_pred = decode(word, char, pos, mask=masks, length=lengths, leading_symbolic=conllx_data.NUM_SYMBOLIC_TAGS, constraints=constraints)
            ...
            stats, stats_nopunc, stats_root, num_inst = parser.eval(word, pos, heads_pred, types_pred, heads, types, word_alphabet, pos_alphabet, lengths, punct_set=punct_set, symbolic_root=True, constraints_mask=None)
	...
	my_log = open('my_log_proj.log','a')
        ...
        print('============================================================================================================================')
        my_log.write('==============================================%s========================================================\n'%(test_path))
        my_log.write('best test W. Punct: ucorr: %d, lcorr: %d, total: %d, uas: %.2f%%, las: %.2f%%, ucm: %.2f%%, lcm: %.2f%% (epoch: %d)\n' % (
            test_ucorrect, test_lcorrect, test_total, test_ucorrect * 100 / test_total, test_lcorrect * 100 / test_total,
            test_ucomlpete_match * 100 / test_total_inst, test_lcomplete_match * 100 / test_total_inst,
            best_epoch))	
        my_log.write('best test Wo Punct: ucorr: %d, lcorr: %d, total: %d, uas: %.2f%%, las: %.2f%%, ucm: %.2f%%, lcm: %.2f%% (epoch: %d)\n\n' % (
            test_ucorrect_nopunc, test_lcorrect_nopunc, test_total_nopunc,
            test_ucorrect_nopunc * 100 / test_total_nopunc, test_lcorrect_nopunc * 100 / test_total_nopunc,
            test_ucomlpete_match_nopunc * 100 / test_total_inst, test_lcomplete_match_nopunc * 100 / test_total_inst,
            best_epoch))
